...
module.exports = class mariadbStore extends session.Store{
	...
	destroy(sid,callback){
		this.runQuery(`DELETE ${this.table} WHERE sid=?`,[],callback)
	}

	clear(callback){
		this.runQuery(`DELETE ${this.table}`,[],callback)
	}
	...
	set(sid,session,callback){
		this.runQuery(`INSERT INTO ${this.table} (sid,session) VALUES (?,?)`,[sid,session],callback)
	}
